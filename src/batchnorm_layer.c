...
void mean_delta_cpu(float *delta, float *variance, int batch, int filters, int spatial, float *mean_delta)
{
    ...
    for(i = 0; i < filters; ++i){
        ...
        mean_delta[i] *= (-1./sqrt(variance[i] + .000001));
    }
}
void  variance_delta_cpu(float *x, float *delta, float *mean, float *variance, int batch, int filters, int spatial, float *variance_delta)
{
    ...
    for(i = 0; i < filters; ++i){
        ...
        variance_delta[i] *= -.5 * pow(variance[i] + .000001, (float)(-3./2.));
    }
}
    ...
    for(j = 0; j < batch; ++j){
        for(f = 0; f < filters; ++f){
            for(k = 0; k < spatial; ++k){
                ...
                delta[index] = delta[index] * 1./(sqrt(variance[f] + .000001)) + variance_delta[f] * 2. * (x[index] - mean[f]) / (spatial * batch) + mean_delta[f]/(spatial*batch);
            }
        }
    }
}
